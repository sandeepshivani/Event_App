...
class _MOCOHomeState extends State<MOCOHome> {
  _addEvent(BuildContext context, Event event) {
    ...
    ScaffoldMessenger.of(context)
        .showSnackBar(const SnackBar(content: Text("Add Event.")));
  }
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        title: const Text("   Event Mangement "),
      ),
      body: Column(
        children: [
          EventListWidget(
            ...
            onPressed: (event) {
              Navigator.push(
                  context,
                  MaterialPageRoute(
                      builder: (context) => MOCOEventInfoView(
                            event: event,
                            onChanged: (event) {
                              setState(() {});
                            },
      ...
      floatingActionButton: FloatingActionButton(
        onPressed: () {
          Navigator.push(
              context,
              MaterialPageRoute(
                  builder: (context) => MOCOCreateEventView(
